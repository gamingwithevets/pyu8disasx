...
def process_ins_param(addr, dis, param, is_lea = False, data_bit_labels = None, lo = False):
	...
	elif type(param) == disas.Pointer and not is_lea and type(param.disp) == disas.Num and type(param.register) == disas.Register and param.register.size == 2:
		if param.disp.bits == 16:
			if param.register.n in (12, 14):
				val = param.disp.get()
				if ((val >= 0 and val <= 0x1f) or (val >= -0x20 and val < 0)):
					param.register.ptr = False
					return case(str(param), lo) + '  ;  Disp16 used instead of Disp6'
			for k, v in dis.data_labels_addr.items():
				if v.get('word', None) == addr:
					if k not in dis.data_labels:
						logging.warning(f'Data address {k:05X} is referenced but no data label was created for it automatically')
						dis.data_labels[k] = f'_d_{k:05X}' if addr >= dis.romwin else f'_unk_{k:05x}'
					offset = v.get('offset', 0)
					s = dis.data_labels[k]
					if offset > 0: s = f'({s}-{offset})'
					s += f'[{case(param.register, lo)}]'
					return s
	elif type(param) == disas.Num and param.bits == 8 and param.imm:
		for k, v in dis.data_labels_addr.items():
			if v.get('byte1', None) == addr: return f'#BYTE1 {dis.data_labels[k]}'
			elif v.get('byte2', None) == addr: return f'#BYTE2 {dis.data_labels[k]}'
...
def log_exc(func, exc):
	_, _, tb = sys.exc_info()
	where = f'{os.path.basename(tb.tb_frame.f_code.co_filename)}:{tb.tb_frame.f_code.co_name}:{tb.tb_lineno}'
	if issubclass(type(exc), OSError):
		if os.name == 'nt':
			if exc.winerror: errno = f'WE{exc.winerror}'
			else: errno = exc.errno
		else: errno = exc.errno
		func(f'[{type(exc).__name__}] {where}: {exc.filename}{", "+exc.filename2 if exc.filename2 else ""}: {exc.strerror} ({errno})')
	else: func(f'[{type(exc).__name__}] {where}: {exc}')

def case(s, lo): return str(s).lower() if lo else str(s).upper()

def disassemble(filename, out, start = 0, extra = None, labelfile = '', dclfile = '', romwin = None, addresses = False, disas_all = False, lo = False):
	...
	try:
		with open(filename, 'rb') as f:
			rom = f.read()
			size = start + len(rom)
			dis = disas.Disassembly(rom, start)
	except Exception as e:
		log_exc(logging.error, e)
		return

	if extra:
		logging.info('Loading extra ROM(s)')
		for _file, _start in extra:
			_start = int(_start, 0)
			try:
				with open(_file, 'rb') as f:
					_rom = f.read()
					size = max(size, _start + len(_rom))
					dis.add_region(_start, _rom)
			except Exception as e:
				log_exc(logging.error, e)
				return

	logging.debug(f'Total ROM size is 0x{size:x}')
	...
	romwin_str = case(f'ROMWINDOW 0, {romwin-1:05X}H', lo) if romwin > 0 else case('NOROMWIN', lo)
	f.write(f'''\
/* =========================================================
This file was generated by PyU8disasX CLI auto-disassembler.
	...
	for seg in range(num_segs):
		...
		for addr16 in range(0x10000):
			...
			if addr < 6:
				...
				elif addr == 2: f.write(f'; Entry point\n{"/*"+tab+"00002"+tab+format(word,"04X")+tab*2+" */ " if addresses else tab}{case("DW", lo)} {process_ins_param(addr, dis, disas.Address(word, 0), lo)}\n')
				elif addr == 4: f.write(f'; BRK interrupt entry point\n{"/*"+tab+"00004"+tab+format(word,"04X")+tab*2+" */ " if addresses else tab}{case("DW", lo)} {process_ins_param(addr, dis, disas.Address(word, 0), lo)}\n')
				skip_byte = 1
				...
				if not table_mode:
					...
					tbytes_line = 0
				word = dis.read_word(addr)
				f.write(f'; Interrupt: {interrupts[addr]}\n{"/*"+tab+format(addr, "05X")+tab+format(word,"04X")+tab*2+" */ " if addresses else tab}{case("DW", lo)} {process_ins_param(addr, dis, disas.Address(word, 0), lo)}\n\n')
				skip_byte = 1
				...
				if entry[1]:
					__size = 0
					for a in range(addr, addr+_size*4, 4):
						word1 = dis.read_word(a)
						word2 = dis.read_word(a+2)
						func_name = process_ins_param(addr, dis, disas.Address(word1, word2))
						f.write(f'{"/*"+tab+format(a, "05X")+tab+format(word1,"04X")+tab*2+" */ " if addresses else tab}{case("DW OFFSET", lo)} ({func_name})\n')
						f.write(f'{"/*"+tab+format(a+2, "05X")+tab+format(word2,"04X")+tab*2+" */ " if addresses else tab}{case("DW SEG", lo)} ({func_name})\n')
						__size += 1
						if a+4 in dis.jump_tables or a+4 in dis.data_labels or a+4 in dis.code:
							_size = __size
							break
					skip_byte = _size * 4 - 1 if table_mode else _size * 2 - 1
					if addr16 + 2 + skip_byte >= 0x10000: logging.warning(f'Skipping {skip_byte if table_mode else skip_byte*2} bytes')
				else:
					s = entry[2]
					__size = 0
					for a in range(addr, addr+_size*2, 2):
						word = dis.read_word(a)
						f.write(f'{"/*"+tab+format(a, "05X")+tab+format(word,"04X")+tab*2+" */ " if addresses else tab}{case("DW", lo)} {process_ins_param(a, dis, disas.Address(word, s), lo)}\n')
						__size += 1
						if a+2 in dis.jump_tables or a+2 in dis.data_labels or a+2 in dis.code:
							_size = __size
							break
					skip_byte = _size * 2 - 1 if table_mode else _size - 1
					if addr16 + 2 + skip_byte >= 0x10000: logging.warning(f'Skipping {skip_byte if table_mode else skip_byte*2} bytes')
				f.write('\n')
				...
				is_lea = instr[0] == 'LEA'
				if len(instr) >= 2: string += ' ' + process_ins_param(addr, dis, instr[1], is_lea, data_bit_labels, lo)
				if len(instr) == 3: string += ', ' + process_ins_param(addr, dis, instr[2], is_lea, data_bit_labels, lo)
				f.write(string + '\n')
				skip_byte = len(instrl) - 1
				if skip_byte > 2: logging.warning(f'Skipping {skip_byte*2} bytes')
			elif addr < size:
				if not table_mode:
					if addr < romwin: f.write(case(f'\nTSEG #{seg} AT {addr:05X}H\n\n', lo))
					else: f.write('\n')
				...
				f.write(case(get_byte(dis.read_byte(addr)), lo))
				tbytes_line += 1
				if tbytes_line == 16:
					f.write('\n')
					tbytes_line = 0
			else:
				logging.warning(f'Skipping addresses {addr:05X}-{((seg+1) << 16) - 1:05X}')
				break
...
if __name__ == '__main__':
	...
	parser.add_argument('start', type = lambda x: int(x, 0), nargs = '?', default = 0, help = 'start address (default: 0)')
	parser.add_argument('-e', '--extra', nargs = 2, metavar = ('FILE', 'START'), action = 'append', help = 'add an extra file')
	...
	if has_labeltool: disassemble(args.file, output, args.start, args.extra, args.label, args.dcl, args.romwin, args.addresses, args.all, args.lowercase)
	else: disassemble(args.file, output, args.start, args.extra, romwin = args.romwin, addresses = args.addresses, lo = args.lowercase)
